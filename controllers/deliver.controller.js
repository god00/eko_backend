...
exports.addNewRoutes = (req, res, next) => {
    try {
        const { routes, cacheForMinimunCost } = deliverService.extractRoute(req.body.input);
        if (routes && cacheForMinimunCost) {
            mapRoutesId[countId] = { routes, cacheForMinimunCost };
            return res.status(200).json({ id: countId++ });
        }
        return res.status(400).json({ errorMsg: 'Invalid parameter' });
    } catch (err) {
        return res.status(400).json({ errorMsg: 'Unexpected Error' });
    }
}
